...
def test_ngspice_identity_uses_first_version_line(monkeypatch):
    calls = []

    def fake_run(argv, **kwargs):
        calls.append((argv, kwargs))
        return SimpleNamespace(returncode=0, stdout="ngspice-44.2\nCopyright\n", stderr="")

    monkeypatch.setattr(subprocess, "run", fake_run)
    assert NgspiceAdapter("my ngspice").identity() == "simcairn-ngspice/1:ngspice-44.2"
...
def test_ngspice_command_artifacts_and_measure_collection(tmp_path):
    ...
    adapter.collect(tmp_path, {"measure_fields": ["gain", "noise", "absent"]})
...
def test_ngspice_collection_requires_log_and_at_least_one_measurement(tmp_path):
    ...
    with pytest.raises(AdapterError, match="none of the requested"):
        adapter.collect(tmp_path, {"measure_fields": ["gain"]})
